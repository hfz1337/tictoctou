...
void gameloop() {
    ...
    for (i = 0; !(result = checkwin()) && i < 9; i++) {
        ...
        while (choice < 49 || choice > 57 || board[choice - 49] != ' '){
            printf("Invalid identifier or cell occupied, please enter a valid number: ");
            choice = getchoice();
        }
...
void how_to_play() {
    ...
    puts("Cells are numbered like shown: ");
    draw_board();
    puts("Just pick the number of the cell you want to mark, ez pz.");
}
...
int save_stats(char *nickname, char *filepath, int result, int num_moves) {
    ...
    for (int j = 0; j < num_moves; j++) {
        if (j % 2) {
            fprintf(f, "# Brainless Bot (X) plays on cell %c          #\n", history[j]);
        } else {
            fprintf(f, "# Human (O) plays on cell %c                  #\n", history[j]);
        }
    }
    fprintf(f, "##############################################\n\n");
